...
class DefaultOrderProvider extends ChangeNotifier {
  late String _street = '';
  ...
  late List<Function> _dryCleanerOptionsIncFunc=[];
  late List<Function> _dryCleanerOptionsDecFunc=[];
  ...
  Future<void> init() async {
   loadServices();
    ...
    _selectionsAddCleanOptions =
        List.generate(_dryCleanerOptionsDataList.length, (index) => false);
    ...
    _dryCleanerOptionsIncFunc =
        List.generate(_dryCleanerOptionsDataList.length, (index) {
      return () {
  ...
  List<Widget> cleanerOptionsList() {
    return List.generate(
        _listServices?.length??0,
            (index) => ServiceWidgetExtend(
          title: _listServices![index]['title'],
          price: '${_listServices![index]['price']}',
          textColor:   const Color.fromRGBO(64, 64, 64, 1),
          backgroundColor: Color.fromRGBO(248, 248, 248, 1),
          image:   SvgPicture.network('${Api.scheme}://${Api.host}${ _listServices![index]['icon']}'
            ,
            // height: 30,
          ),
        ));
    ...
    return List.generate(
        ...
        (index) => ServiceWidgetExtend(
              ...
              image: (selectionsAddCleanOptions[index] == true) &&
                      (!_dryCleanerOptionsDataList[index]['isSingle'])
                  ? CounterSecondaryWidget(
                      _dryCleanerOptionsCount[index],
                      onIncrementTap: _dryCleanerOptionsIncFunc[index],
                      onDecrementTap: _dryCleanerOptionsDecFunc[index],
                    )
                  : SvgPicture.asset(
