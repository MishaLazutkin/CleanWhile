import 'package:clean_whale/widgets/window_widget.dart';
...
class WindowProvider extends ChangeNotifier {
  ...
  late List<bool> _selectionsAddCleanOptions;
  ...
  late List<Function> _addCleanerOptionsIncFunc;
  late List<Function> _addCleanerOptionsDecFunc;

  late List<int> _cleanerOptionsCount;
  late List<int> _handyManOptionsCount;
  late List<int> _addCleanerOptionsCount;
  late List<Map<String, dynamic>> _windowRulesDataList;

  late bool isChecked = false;
  late bool _isExpandedCalcRules = false;
  late bool _isExpandedAdditionalOptions = false;
  ...
  Future<void> init() async {
    ...
    _windowRulesDataList = [
      {
        'title': 'Ось це двостулкові вікно, воно вважається як одне вікно',
        'uriPicture': 'lib/assets/icons/windows_1.png',
      },
      {
        'title': 'Одностулкове вузьке вікно - це половина вікна',
        'uriPicture': 'lib/assets/icons/windows_2.png',
      },
      {
        'title': 'Якщо у вас три стулки, це 1,5 вікна',
        'uriPicture': 'lib/assets/icons/windows_3.png',
      },
      {
        'title': 'Балконні двері і вікно -це 2 вікна',
        'uriPicture': 'lib/assets/icons/windows_4.png',
      },
      {
        'title':
            'Одна секція до підлоги рахується як одне вікно. На зображенні 5 вікон.',
        'uriPicture': 'lib/assets/icons/windows_5.png',
      },
      {
        'title':
            'Якщо вікна не до підлоги, вони рахуються за шибками. На картинці 2 вікна (4 шибки)',
        'uriPicture': 'lib/assets/icons/windows_6.png',
      },
    ];
    ...
    _addCleanerOptionsIncFunc =
        List.generate(_dryCleanerDataList.length, (index) {
      return () {
        _addCleanerOptionsCount[index]++;
        notifyListeners();
      };
    });

    _addCleanerOptionsDecFunc =
        List.generate(_dryCleanerDataList.length, (index) {
      return () {
        if (_addCleanerOptionsCount[index] > 1) {
          _addCleanerOptionsCount[index]--;
        }
        notifyListeners();
      };
  ...
  List<Widget> discountList() {
    return List.generate(
        ...
        (index) => Discount(
              ...
              backgroundColor: selectedDiscountId == index
                  ? AppColor.primary
                  : const Color.fromRGBO(255, 255, 255, 1),
            ));
  }

  List<Widget> windowsRulesList() {
    return List.generate(
        _windowRulesDataList.length,
        (index) => WindowWidget(
              title: _windowRulesDataList[index]['title'],
              picUri: _windowRulesDataList[index]['uriPicture'],
            ));
  }
  ...
  List<Widget> addCleanerOptionsList() {
    return List.generate(
        ...
        (index) => ServiceWidgetExtend(
              ...
              textColor: selectionsAddCleanOptions[index] == true
                  ? Colors.white
                  : const Color.fromRGBO(64, 64, 64, 1),
              backgroundColor: selectionsAddCleanOptions[index] == true
                  ? AppColor.primary
                  : const Color.fromRGBO(248, 248, 248, 1),
              image: (selectionsAddCleanOptions[index] == true) &&
                      (!_dryCleanerDataList[index]['isSingle'])
                  ? CounterSecondaryWidget(
                      _addCleanerOptionsCount[index],
                      onIncrementTap: _addCleanerOptionsIncFunc[index],
                      onDecrementTap: _addCleanerOptionsDecFunc[index],
                    )
                  : SvgPicture.asset(
  ...
  List<bool> get selectionsAddCleanOptions => _selectionsAddCleanOptions;

  setSelectionsAddCleanOptions(int index, bool value) {
    _selectionsAddCleanOptions[index] = value;
    notifyListeners();
  }
  ...
  bool get isExpandedCalcRules => _isExpandedCalcRules;

  set isExpandedCalcRules(bool value) {
    _isExpandedCalcRules = value;
    notifyListeners();
  }
  ...
  List<Map<String, dynamic>> get windowRules => _windowRulesDataList;

  bool get isExpandedAdditionalOptions => _isExpandedAdditionalOptions;

  set isExpandedAdditionalOptions(bool value) {
    _isExpandedAdditionalOptions = value;
    notifyListeners();
  }
  ...
  int get balconiesCount => _balconiesCount;

  set balconiesCount(int value) {
    _balconiesCount = value;
    notifyListeners();
  }
}
